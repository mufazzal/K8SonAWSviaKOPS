...
# scriptRoot="https://k8shn-muf-k8s-kops-work-space-bucket.s3.amazonaws.com/Scripts"
scriptRoot="${scriptRoot}"
# https://muf-k8s-kops-work-space-bucket.s3.amazonaws.com/Scripts -- install_dependencies.sh -- install_dependencies.sh


loadScript() {
    scriptName=$1
    scriptFileName=$(sed 's/\//-/g' <<<"$scriptName")
    echo "$scriptRoot -- $scriptName -- $scriptFileName"
...
init() {
    ...
    loadScript "util.sh"
    doCommonConfig ${state_bucket} ${cluster_name} ${scriptRoot}
